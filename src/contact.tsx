...
function ContactPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!supabase) {
      setStatus("error");
      setError("Contact form is not configured yet. Please email admin@oncuemarketing.info.");
      return;
    }
    setStatus("loading");
    ...
    <div className="bg-black text-white">
      ...
      <section className="max-w-7xl mx-auto px-6 py-20 grid md:grid-cols-12 gap-12">
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-lg sm:text-xl font-bold group-hover:text-[var(--color-gold)] transition break-words">{c.value}</p>
                </div>
              </div>
            </a>
          ))}
        ...
        <form onSubmit={onSubmit} className="md:col-span-7 border border-white/15 p-8 md:p-10 space-y-6">
          <h2 className="text-xl sm:text-2xl md:text-3xl font-black uppercase tracking-wide leading-tight break-words">Send us a brief</h2>
          <div>
